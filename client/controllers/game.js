angular.module('Music-Dictators').controller('gameCtrl', function($scope, $modal, socket, $location, $auth, $window) {
	...
	socket.on('game over', function(game) {
		if (game.player1.id === $scope.user._id) {
			$scope.myResults = game.player1;
			$scope.oponentResults = game.player2;
		} else {
			$scope.myResults = game.player2;
			$scope.oponentResults = game.player1;
		}
		gameRuning = false;
		myModal = $modal({
			title : 'Game Over',
